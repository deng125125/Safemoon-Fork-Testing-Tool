...
contract('Kodi (dividend)', (accounts) => {
  ...
  // type of contract
  const hasDividend = true;  // is dividend paying token or not
  const hasAddLiquidityFunction = true;  // has addLiquidity() function or not

  // dividend-paying-token only
  const isDividendBNB = true;  // true if dividend is BNB?
  let dividendTokenAddress;  // if isDividendBNB is false, give this parameter an address
  const minimumTokenBalanceForDividendsinFinney = 5000000000;  // in finney, 1 ether == 1000 finney
  ...
  const isAllFeeSentToContract = true;

  // fees to be charged when addLiquidity(), 0 for most projects
  let walletFee = 0;

  // standard transfer Fees
  const dividendRewardFeeRate = 4; // dividend-paying-token only
  const reflectFeeRate = 0;  // 0 for dividend-paying-token
  const liquidityFeeRate = 2;
  const marketingFeeRate = 8;
  const burnFeeRate = 0;
  const buyBackFeeRate = 6;
  
  //separate buy Fees
  const hasSeparateBuyFees = true;  // true if contract has separate buy fees, and change the following values if true
  const buyDividendRewardFeeRate = 2; // dividend-paying-token only
  const buyReflectFeeRate = 0;
  const buyLiquidityFeeRate = 1;
  const buyMarketingFeeRate = 4;
  const buyBurnFeeRate = 0;
  const buyBuyBackFeeRate = 3;
  ...
  const numToAddliquidityinFinney = 25000000000;
    ...
    it('Buy tokens from Router', async () => {
      ...
      it('Sell tokens to Router', async () => {
        ...
        assert.ok(delta1 * 1000 / fees < 1, 'fees are not sent to the contract when sell, or parameter isAllFeeSentToContract are not set correctly');  // allow 0.1% diff
          ...
          // Kodi failed in this test, error message: "SafeMath: division by zero Modified",
    // at line 1354 uint256 bnbForMarketing = bnbBalance.mul(_marketingTokensToSwap).div(totalTokensToSwap);
    // because totalTokensToSwap is 0
