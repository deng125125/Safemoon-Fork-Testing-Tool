...
contract('Unimoon', (accounts) => {
  ...
  const reflectFeeRate = 4;  // 0 for dividend-paying-token
  const liquidityFeeRate = 3;
  const marketingFeeRate = 5;
  const burnFeeRate = 2;
  const buyBackFeeRate = 1;
  ...
  //separate sell Fees
  const hasSeparateSellFees = true;  // true if contract has separate sell fees, and change the following values if true
  const sellDividendRewardFeeRate = 0;
  const sellReflectFeeRate = 8;
  const sellLiquidityFeeRate = 6;
  const sellMarketingFeeRate = 10;
  const sellBurnFeeRate = 2;
  const sellBuyBackFeeRate = 2;
...
//   const numToAddliquidityinFinney = 500000;   // 500000 finney == 500 ether
  const numToAddliquidityinFinney = 4 * 10**6;
...
//     // enable trading and set fees
//     await SafemoonInstance.methods.tradingStatus(true).send({from: deployer});
//     await SafemoonInstance.methods.set_All_Fees(100,200,300,400,500).send({from: deployer});
